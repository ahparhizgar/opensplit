...
import com.opensplit.component.apiCallScope
...
interface AuthComponent {
    ...
    interface Factory {
        fun create(context: CContext): AuthComponent
    }
}
...
class DefaultAuthComponent(
    ...
    override fun useSignIn() {
        _uiState.update { it.copy(mode = AuthMode.SignIn, fieldErrors = emptyMap(), generalError = null) }
    }

    override fun useSignUp() {
        _uiState.update { it.copy(mode = AuthMode.SignUp, fieldErrors = emptyMap(), generalError = null) }
    }

    override fun updateEmail(email: String) {
        _uiState.update { it.copy(email = email, fieldErrors = it.fieldErrors - "email", generalError = null) }
    }

    override fun updatePassword(password: String) {
        _uiState.update { it.copy(password = password, fieldErrors = it.fieldErrors - "password", generalError = null) }
    }
    ...
    override suspend fun submit() {
        ...
        val validation = when (current.mode) {
            AuthMode.SignIn -> com.opensplit.validation.auth.AuthValidation.validateSignIn(current.email, current.password)
            AuthMode.SignUp -> com.opensplit.validation.auth.AuthValidation.validateSignUp(current.email, current.password)
        }

        if (!validation.isValid) {
            _uiState.update { it.copy(fieldErrors = validation.errors, generalError = null, session = null, isSubmitting = false) }
            return
        }
...
class FakeAuthComponent(
    uiState: AuthViewState = AuthViewState(
        session = com.opensplit.dto.auth.AuthSessionState(userId = "user-1", email = "amir@example.com", accessToken = "token")
    )
) : AuthComponent {
