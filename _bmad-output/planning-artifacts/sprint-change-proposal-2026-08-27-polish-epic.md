...
Four technical debt and UX refinement tasks identified during Epic 1 and Epic 2 implementation need to be addressed **before Epic 3 (Balances & Settlement)** begins. These tasks represent critical polish work on the group setup workflow, logout functionality, and UI consistency.
...
- ✅ Improves UX clarity (refactored join group flow)
- ✅ Polishes UI consistency (form styling across group setup)
...
3. **UI Discoverability:** Join group workflow is not clearly visible on group list
4. **UI Inconsistency:** Group creation and join forms lack polish and don't match app design system

**Why This Matters:**
- Users need logout for security, especially on shared/group devices
- Clear group setup UX is foundation for all future work
- Polished forms improve user confidence and reduce errors
- Better to address now than accumulate post-MVP technical debt

**Discovery Context:**
- Epic 1 (Join Your Group Securely) marked complete with 9 stories done
...
| 1-4 (Manage membership) | 5-3 depends on it | Join button added to group list |
...
|----------|--------|------------------|
| **PRD** | No changes needed | v1 scope already included logout and group UX polish |
...
| **UX Design** | Minor update | Group forms styling documentation |
...
- **Navigation:** Group setup flow refactored (back buttons, navigation patterns)
...
- Story 5-3 (Join Group UI): 3-4 days
- Story 5-4 (Create Group UI): 3-4 days
...
- ✅ Team has experience with group workflows (from Epic 1)
...
**Problem:** Users cannot sign out from the app. On shared/group devices, this is a security risk.
...
**UX Changes:**
- Add logout button to group/settings screen (typically top-right menu or profile)
...
1. Given a user is viewing the groups list screen
...
5. Given the user is back on the group list after joining
   When they view their groups
   Then the newly-joined group appears in their list
```

**Technical Notes:**
- Refactor navigation flow: remove combined component, add join button to group list
...
- Add "Join an Existing Group" button to group list screen, below "Start a New Group"
...
**Problem:** Join group form lacks visual polish and consistency with app design system.
...
**Problem:** Create group form needs visual polish to match app design system and provide better feedback.
...
3. → QA: Regression testing for group workflows
...
- [ ] Zero regressions in group workflows (Epic 1/2 unchanged)
- [ ] Form UI polish matches app design system (ThemeExtended)
- [ ] QA regression suite passes (group creation, joining, logout)
...
3. **Story 5-3 (Join Group UI)** — Depends on 5-2 completion
4. **Story 5-4 (Create Group UI)** — Can run parallel with 5-3 (isolated from join flow)
...
**Rationale:** 
- Logout and group UX polish are part of v1 scope (per PRD)
...
- ✅ Action Item Follow-Through: Polish work directly addresses "improve group workflows" need identified during Epic 2

### Connection to Epic 1 Completion

Epic 5 improves the group setup workflows from Epic 1:
...
- ✅ Assign experienced developer familiar with group workflows
...
- ✅ Group workflows solid (Epic 5 polish)
